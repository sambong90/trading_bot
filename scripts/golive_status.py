...
def main():
    ...
    try:
        closed = s.execute(text(
            "SELECT regime, roi_realistic_pct, roi_signal_pct, exit_reason "
    ...
    # ── 기준 3: PF (전체 + 국면별) ──
    print("\n[기준3] PF (전체 >=1.2 · 각 국면 >=1.0)")
    all_rois = [roi_of(r) for r in closed]
    pf_all = pf(all_rois)
    c3_all = pf_all >= PF_OVERALL_MIN
    print(f"   전체     n={n_closed:4d}  PF={fmt_pf(pf_all)}  (>=1.2) {'OK' if c3_all else 'X'}")
    c3_reg = True
    for rg in REGIMES:
        rois = [roi_of(r) for r in by_reg[rg]]
        if not rois:
            print(f"   {rg:9s} n=   0  PF=  n/a  (표본 없음 — 미충족)")
            c3_reg = False
            continue
        p = pf(rois); ok = p >= PF_REGIME_MIN
        c3_reg &= ok
        print(f"   {rg:9s} n={len(rois):4d}  PF={fmt_pf(p)}  (>=1.0) {'OK' if ok else 'X'}")
    c3_ok = c3_all and c3_reg
    print(f"   => 기준3 {'충족' if c3_ok else '미충족'}")
